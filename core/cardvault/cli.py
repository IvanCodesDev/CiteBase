...
from cardvault import __version__, evalrun, frontmatter, retrieve, spanhash
...
def cmd_index(args: argparse.Namespace) -> int:
    ...
    if vault.config.index_backend == "sqlite":
        from cardvault.backends import write_sqlite
...
def cmd_ingest(args: argparse.Namespace) -> int:
    from cardvault.adapters import adapter_for_path
    from cardvault.ingest import ingest
...
def cmd_compile(args: argparse.Namespace) -> int:
    from cardvault.compiler import compile_vault, load_scripted
    from cardvault.compiler.openai_compat import LlmUnavailableError, OpenAICompatProvider
    from cardvault.ports import LlmProvider
...
def cmd_review(args: argparse.Namespace) -> int:
    from cardvault.compiler import review as review_mod
...
def cmd_init(args: argparse.Namespace) -> int:
    from cardvault.scaffold import init_vault
...
def cmd_gaps(args: argparse.Namespace) -> int:
    from cardvault.retrievelog import gap_report
...
def cmd_deps(args: argparse.Namespace) -> int:
    from cardvault.federation import FederationError, deps_status, deps_sync
...
def cmd_export(args: argparse.Namespace) -> int:
    ...
    out = Path(args.out)
    if args.what == "json":
        from cardvault.exporters import export_json
        ...
        return 0
    from cardvault.exporters import export_site
...
def cmd_bench(args: argparse.Namespace) -> int:
    ...
    from cardvault.bench import run_bench

    with tempfile.TemporaryDirectory(prefix="cardvault-bench-") as tmp:
        report = run_bench(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="vault",
        description="CardVault：编译式知识库（M0：无 LLM 的 lint / 索引 / 检索）",
    )
    parser.add_argument("--version", action="version", version=f"cardvault {__version__}")
