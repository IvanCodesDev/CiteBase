...
from cardvault import frontmatter, security, spanhash
...
def _load_card_schema() -> dict[str, Any]:
    text = resources.files("cardvault").joinpath("spec/card.schema.json").read_text("utf-8")
...
class _FederationContext:
    ...
    def lock(self) -> dict[str, Any]:
        if self._lock is None:
            from cardvault.federation import load_lock
    ...
    def dep_vault(self, dep_id: str) -> Vault | None:
        if dep_id not in self._dep_vaults:
            from cardvault.federation import resolver_for
